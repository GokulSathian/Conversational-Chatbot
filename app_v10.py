                ...
                with st.expander(f"User: {row['username']}", expanded=False):
                    ...
                    with col2:
                        if st.button("Reset Prompt Count", key=f"reset_{row['username']}"):
                            # Reset prompts_used to 0 and max_prompts to 50
                            result = user_settings_collection.update_one(
                                {"username": row['username']},
                                {"$set": {"prompts_used": 0, "max_prompts": 50}},  # Reset to 50
                                upsert=True
                            )
                            if result.modified_count > 0 or result.upserted_id is not None:
                                st.success("Prompt count and limit reset to 50 successfully!")
                                st.rerun()  # Force refresh the UI
                            else:
                                st.error("Failed to reset prompt count")
                    with col3:
                        if st.button("Delete User", key=f"delete_{row['username']}"):
                            # Delete user from users_collection
                            users_collection.delete_one({"username": row['username']})
                            # Delete user settings
                            user_settings_collection.delete_one({"username": row['username']})
                            # Delete user chats
                            chats_collection.delete_many({"username": row['username']})
                            # Delete vector stores if they exist
                            vector_store_path = f"vector_stores/{row['username']}"
                            if os.path.exists(vector_store_path):
                                import shutil
                ...
                if not st.session_state.stop_signal:
                    ...
                    if chat["vector_store"] is not None:
                        ...
                        qa_chain = ConversationalRetrievalChain.from_llm(
                            ...
                            memory=chat["qa_memory"],  # Use the QA-specific memory
                        ...
                        # Ensure we only access the expected output
                        if "answer" in result:
                            response = result["answer"]  # Access the answer from the result
                            chat["chat_history"].append((prompt, response))
                        else:
                            response = "No answer found."  # Handle case where answer is not present
                    else:
                        llm = ChatOpenAI(model="gpt-4o-mini", openai_api_key=OPENAI_API_KEY)
